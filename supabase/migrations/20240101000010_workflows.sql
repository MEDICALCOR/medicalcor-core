-- ============================================================================
-- MedicalCor Core - Workflows Schema
-- ============================================================================
-- Source: infra/migrations/006-workflows.sql
...
CREATE TABLE IF NOT EXISTS workflows (
    ...
    CONSTRAINT valid_trigger_type CHECK (
        trigger_type IN ('new_lead', 'appointment_scheduled', 'appointment_completed',
            'no_response', 'message_received', 'tag_added', 'status_changed')
    )
);
...
('Bun venit Lead Nou', 'Trimite mesaj de bun venit automat la lead-uri noi', 'Lead Management', 'new_lead',
 '[{"id":"s1","type":"action","action":{"id":"a1","type":"send_whatsapp","config":{"template":"welcome"}}}]'::jsonb, true),
('Follow-up Post Consultație', 'Trimite mesaj de satisfacție la 24h după consultație', 'Patient Care', 'appointment_completed',
 '[{"id":"s1","type":"delay","delay":{"value":24,"unit":"hours"}},{"id":"s2","type":"action","action":{"id":"a1","type":"send_whatsapp","config":{"template":"followup"}}}]'::jsonb, true),
('Reminder Programare 24h', 'Trimite reminder cu o zi înainte de programare', 'Appointments', 'appointment_scheduled',
 '[{"id":"s1","type":"action","action":{"id":"a1","type":"send_whatsapp","config":{"template":"reminder_24h"}}}]'::jsonb, true)
