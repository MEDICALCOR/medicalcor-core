-- =============================================================================
-- 07-crm-hardening.sql
-- Constraints, Foreign Keys, Checks & Indexes for 06-crm.sql
-- =============================================================================

-- 1. LEADS - FKs, Checks, Unique
DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM information_schema.tables WHERE table_name = 'clinics') THEN
        IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'leads_clinic_fk') THEN
            ALTER TABLE leads
                ADD CONSTRAINT leads_clinic_fk
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'leads_assigned_agent_fk') THEN
        ALTER TABLE leads
            ADD CONSTRAINT leads_assigned_agent_fk
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leads_language_valid') THEN
        ALTER TABLE leads
            ADD CONSTRAINT leads_language_valid
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'leads_external_contact_unique') THEN
        ALTER TABLE leads
            ADD CONSTRAINT leads_external_contact_unique
            UNIQUE (external_source, external_contact_id);
    END IF;
END $$;

-- 2. INTERACTIONS - Checks & Unique
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'interactions_sentiment_range') THEN
        ALTER TABLE interactions
            ADD CONSTRAINT interactions_sentiment_range
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'interactions_provider_external_unique') THEN
        ALTER TABLE interactions
            ADD CONSTRAINT interactions_provider_external_unique
            UNIQUE (provider, external_id);
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_interactions_provider_external
    ON interactions(provider, external_id);

-- 3. TREATMENT PLANS - FKs, Checks, Unique
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.table_constraints WHERE constraint_name = 'treatment_plans_doctor_fk') THEN
        ALTER TABLE treatment_plans
            ADD CONSTRAINT treatment_plans_doctor_fk
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'treatment_plans_probability_range') THEN
        ALTER TABLE treatment_plans
            ADD CONSTRAINT treatment_plans_probability_range
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'treatment_plans_total_value_non_negative') THEN
        ALTER TABLE treatment_plans
            ADD CONSTRAINT treatment_plans_total_value_non_negative
...
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'treatment_plans_external_deal_unique') THEN
        ALTER TABLE treatment_plans
            ADD CONSTRAINT treatment_plans_external_deal_unique
            UNIQUE (external_deal_id);
    END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_treatment_plans_lead_stage
    ON treatment_plans(lead_id, stage);

-- 4. TREATMENT_PLAN_ITEMS - Checks & Index
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tpi_quantity_positive') THEN
        ALTER TABLE treatment_plan_items
            ADD CONSTRAINT tpi_quantity_positive
            CHECK (quantity > 0);
    END IF;

    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'tpi_price_non_negative') THEN
        ALTER TABLE treatment_plan_items
            ADD CONSTRAINT tpi_price_non_negative
...
-- 5. LEAD_EVENTS - Index & Check
CREATE INDEX IF NOT EXISTS idx_lead_events_timeline
    ON lead_events(lead_id, event_type, created_at DESC);

DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'lead_events_type_valid') THEN
        ALTER TABLE lead_events
            ADD CONSTRAINT lead_events_type_valid
            CHECK (length(trim(event_type)) > 0);
    END IF;
END $$;

-- 6. PRACTITIONERS - Indexuri
CREATE INDEX IF NOT EXISTS idx_practitioners_external_user
    ON practitioners(external_user_id);

CREATE INDEX IF NOT EXISTS idx_practitioners_role_active
    ON practitioners(role, is_active)
