...
export function exhaustiveMatch<
  ...
  R,
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters -- M enforces complete matchers at compile time
  M extends { [K in T['_tag']]: (value: Extract<T, { _tag: K }>) => R },
>(value: T, matchers: M): R {
  const tag = value._tag;
  const matcher = matchers[tag as keyof M];
  // This is a runtime safety check - compile-time M constraint ensures all tags are covered
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-condition -- runtime safety for dynamic tag values
  if (!matcher) {
...
export function matchWithDefault<
  T extends { _tag: string },
  R,
  // eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters -- M provides type-safe partial matchers
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unnecessary-type-parameters -- T is caller-specified return type
